...
object FlatReplaceFirstTests extends SuiteClass[NoEffect] {
  ...
  val runSuite: Suite[NoEffect] = suite("FlatReplaceFirstTests") {
    tests(
      lazyTest("if PartialFunction.empty, the function is identity") {
      ...
      lazyTest("if the function matches all elements, the head is changed") {
      ...
      lazyTest("if the function matches one of the unique elements, others are left unchanged") {
      ...
      lazyTest("if the function matches more than one element, only the first one is changed") {
